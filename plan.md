...
## 7. 2026-09-09：新授权设备的蜂窝 IMS 与 beta3 验收

此节是另一台设备的独立测试，不替代上节 beta2/VoWiFi 设备的证据。
详细逐卡台账、脱敏结果与备份位置保留在本地，不提交访问凭据或完整用户标识。

- [x] 完整封存原安装，按用户要求使用本仓库全新配置和数据库，不迁移原版配置。
- [x] 修复原生端点准备、紧凑双地址 AT 解析及 WDS/SIP 地址族不一致问题。
- [x] 按 TS 24.229 §5.1.1.2.2 修正标准派生 LTE 初始 Authorization；
  保留明确配置的兼容路径，不放开真实认证失败后的暴力重试。
- [x] `bc0a960` 与 `65b8a0e` 的 Actions 编译、回归及双架构构建通过。
  本轮开发分支只生成经校验的候选 artifact，不覆盖已发布 beta2 tag/资产。
- [x] `65b8a0e` 完成正常初始注册，并等待 3000 秒正常调度后原 UDP 通道续期成功；
  refresh_count 0→1，会话起始时间、重连计数和主服务进程不变。
- [x] 四个官方可下载 catalog 分别实机激活；本卡均依赖标准派生兜底，
  修复后均初始注册成功，不篡改 unknown 为 ready，不冒充库专属参数通过。
- [x] SMS/Trunk 的仅 VoWiFi 限制已开启并保留；没有实际短信发送或拨号。
- [x] 实机验收后将 VERSION、Cargo.toml/Cargo.lock、前端版本统一为 `1.1.4-beta3`。
- [x] 修复分支合并至主分支 `05de680`；最终发布 run `34281100059` 全通过，
  tag `v1.1.4-beta3` 与程序 commit 一致，pre-release=true、非 latest。
- [x] 正式 beta3 发布包完成自动初始注册，并于 06:37:24 独立通过自身的
  原 UDP 通道自然续期：CSeq=3、refresh_count 0→1、重连计数和进程不变，
  续得 3600 秒租期；最终程序校验、配置保持、资费开关及无活动通话复核通过。
- [x] 文档收尾使用 `[skip ci]`，不重建或覆盖 beta3 发布资产；
  Release 和设备程序继续对应 `05de680`。

当前网络没有确认 Outbound 多注册；本次实机结果是单路蜂窝 UDP IMS，不是
IPsec 原 SA 或双注册验收。真实通话、短信业务、账单与浏览器交互仍不标为实测。
