...
- 用户随后允许暂时转向 1.1.5 的 M0/M1，并新增飞行模式/开机连接控制需求；应区分发现与控制基带、射频/驻网、普通数据和 IMS 意图。开发在 `dev/1.1.5-modem-backends` 的独立 worktree，IMS 修复线保留。本条为开发方向，不表示这些设置已实现或当前设备已改变。
- 多 agent 的当前约定是**一名开发者、其余只读报告**，不是多个写入者。恢复 IMS 派生测试时读取用户安全提供的本地专项交接材料，其中包含独立基线、真正兜底的判定、分支与设备交接规则；不要假定 clone 会带上它或其中的凭据。
...
若本次只负责IMS派生/兜底测试或只读审阅，读取用户安全提供的本地专项交接文件；
该文件不随Git分发，不能假定clone包含它或私密凭据。
只有用户指定的唯一开发agent可以修改；其他agent只读并汇总问题。
...
### OWN-SIM-A / 2026-09-13 / Native T01～T03 — 自有设备接管验证

- 新授权对象为用户自有 MSM8916 设备，与朋友 Cloudflare 设备的 SIM-01～03 分开。
- 归属 26202 / 访问 50219，LTE roaming；初始 Native 候选 `b94c9c2`。
- MM 停止情况下发现/稳定身份/UE 隔离及飞行模式开关、恢复驻网已通过。
- 初次 IMS 尝试能解析 derived 归属 profile，但失败在 WDS bearer 前段；
  无 AKA、SIP 初始注册、自然续期、电话/短信通过证据。
- 有界探针定位了 QMI proxy 最后连接退出导致物理通道关闭、CID 失效，以及地址族
  CLI 参数错误；保持 proxy-open lease 后，同 CID 配置/状态/释放探针通过。
- 代码修复、CI、后续部署和未覆盖范围见
