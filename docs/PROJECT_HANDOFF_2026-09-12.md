...
只做 IMS 派生/兜底分析或恢复测试，可直接读取 [独立测试交接](IMS_DERIVED_FALLBACK_HANDOFF.md)。
...
- 多 agent 的当前约定是**一名开发者、其余只读报告**，不是多个写入者。恢复 IMS 派生测试时读取 `IMS_DERIVED_FALLBACK_HANDOFF.md`，其中包含独立基线、真正兜底的判定、分支与设备交接规则。
...
若本次只负责IMS派生/兜底测试或只读审阅，直接阅读docs/IMS_DERIVED_FALLBACK_HANDOFF.md。
