...
| 1.1.5 | 统一设备接口；兼容 MM 与原生 QMI/MBIM/AT，MM 从必要条件变成可选后端 | 原生逻辑候选 `b94c9c2` 已过 CI，默认 MM、未部署、实机延期；混合 owner/代次恢复等仍待完成，见原生后端状态文档 |
...
- 规划阶段本身不改版本号或设备；后续 M0/M1 已在独立开发分支推进，当前没有部署、卸载/停止 MM，也不把候选代码当作已通过实机的正式功能。
- 后续用户已授权开始 1.1.5 的 M0/M1，与 IMS 修复线分目录维护；新增启动/飞行模式/自动连接控制需求见版本规划第 4.3 节。只有一名开发 agent 修改，其余只读汇总；IMS 专项交接材料按用户要求仅在本地保存，换环境时由用户安全提供，不依赖 Git 分发。
