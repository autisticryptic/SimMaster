...
用户确认 **SIM-04 已验证正常自然续期，SIM-05 已手动测试完成**。这两项不再作为等待事项，
不重复主动轮询或重新部署以补验收。本次接续未重新读取现场，也不补造特定 PID/版本的原始证据。
**SIM-06 中国电信 IMS 注册失败** 已列为当前 native 功能及 CI/文档收尾后的下一项目标。
这些反馈不能替代 native 后端的独立硬件验收。
...
### 4.2 自然续期观察（历史采样；已按用户确认收口）

本地只读观察器已取得 11:04:30、11:09:43 两次同会话样本，随后按用户要求停止，
以继续 native 接口补强和分支整理；**没有停止或回滚设备服务**。租期由设备正常维护。
当时的验收工具要求同一进程/namespace/socket、明确 refresh 成功、CSeq 增长及新租期调度，
解析器兼容带引号的 `register_phase`，四项离线正/负回归通过。
本次用户已确认 SIM-04 自然续期完成，不再等待或要求重做；不把这份历史采样冒充新取得的日志。

## 5. 尚未完成的边界

- native 端到端硬件验收仍未完成。通话/注册事件、Quectel/DJI 维护、SIM 通道账本及直接短信
  已有后继实现，代码/CI 的逐项状态见原生审计，不沿用早期“均未实现”的结论。
- 当前下一项目标是 SIM-06 中国电信 IMS 注册失败；SIM-04/05 不再列为待重复验收。
