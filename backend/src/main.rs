...
enum CliCommand {
    ...
    DeviceInit {
        ...
        /// Report what would happen without changing device state.
        #[arg(long)]
        dry_run: bool,
    },
    /// Install resources owned by the detected device driver.
    InstallDeviceResources {
        /// Unpacked package root containing devices/<device>/system.
        #[arg(long, default_value = ".")]
        staging_dir: String,
        /// Activate installed services immediately instead of deferring to boot.
        #[arg(long)]
        activate: bool,
    },
...
async fn main() -> Result<()> {
    ...
    {
        return hardware::devices::run_native_bearer_init(
    ...
    if let Some(CliCommand::InstallDeviceResources {
        staging_dir,
        activate,
    }) = &cli.command
    {
    ...
    let line_registry = Arc::new(
        services::line_registry::LineRuntimeRegistry::with_config_for_device(
...
///
/// The device data transport moves its native netdev back out of the UE
...
async fn release_data_sessions(line_registry: &services::line_registry::LineRuntimeRegistry) {
    ...
    let released = tokio::time::timeout(BUDGET, async {
        for line in lines {
            let line_id = line.binding().line_id;
            if line.cellular_data.interface().await.is_none() {
                continue;
            }
...
mod http_router_tests {
    ...
    async fn a_partial_put_body_is_refused_by_the_live_endpoint() {
        ...
        assert_eq!(
            status,
            StatusCode::OK,
            "summary list must answer: {summaries}"
        );
        assert!(
        ...
        assert_eq!(
            status,
            StatusCode::OK,
            "database detail must answer: {detail}"
        );
        assert!(
