    ...
    if rules.is_empty() {
        ...
        let body = format!(
            "# Generated by `simadmin device-init`.\n\
             # DATA6 is a SimAdmin-created ordinary-data endpoint;\n\
             # ModemManager must not claim it as an extra modem port.\n{}\n",
            rules.join("\n")
        );
...
pub async fn run(write_udev_rule: bool, dry_run: bool) -> Result<()> {
    ...
    for primary in &primaries {
        if dry_run {
            match secondary_qmi::baseband_key_for_device(primary) {
                Ok(baseband) => {
                    println!(
                        "would prepare the DATA6 data endpoint for {primary} (baseband {baseband})"
                    )
                }
    ...
    println!(
        "device-init(qcm410): {} of {} baseband(s) have a DATA6 data endpoint",
        prepared.len(),
        primaries.len()
    );
    if prepared.is_empty() {
        println!("device-init(qcm410): DATA6 unavailable; ordinary native data is unavailable");
        return Ok(());
    }
