...
use crate::{
    ...
    services::trunk::{
        bridge::{
            ...
            OperatorCommand, OperatorEvent, RtpTelephoneEvent,
        },
        operator::OperatorLink,
    },
};

use super::channel::{EpdgSipChannel, SipChannel};
    ...
    let frame = match command {
        OperatorCommand::StartCall {
            ...
            if offer.video.is_some() {
                return Err("vowifi_video_not_supported".into());
            }
            ...
            let internal_local = pending
                .internal_local_addr()
                .map_err(|error| format!("vowifi_rtp_local_failed:{error}"))?;
            let body = relay_media_sdp(&offer, operator_local);
...
async fn begin_incoming_call(
    ...
    let caller = normalize_caller(
        &header_uri(frame, "P-Asserted-Identity").unwrap_or_else(|| remote_uri.clone()),
    );
    ...
    if operator_answered {
        let answer = relay_media_sdp(&offer, operator_local);
    ...
    let trunk_offer = relay_media_sdp(&offer, internal_local);
    session.calls.insert(
        trunk_call_id.clone(),
        VoiceCall {
    ...
    internal_answer.codecs = operator_audio
        ...
        .filter_map(|operator| {
            call.internal_offer
                ...
                .codecs
                .iter()
                .find(|internal| internal.codec == operator.codec)
                .cloned()
        })
        .collect();
...
fn prepare_incoming_media(
    ...
    operator_answer.codecs.retain(|operator| {
        internal_audio
            .codecs
            .iter()
...
fn relay_media_sdp(offer: &MediaOffer, local: SocketAddr) -> String {
    relay_audio_sdp(&offer.audio, offer.dtmf.rtp_event.as_ref(), local)
}
...
mod tests {
    ...
    async fn incoming_invite_round_trips_answer_and_remote_bye() {
        ...
        assert_eq!(caller, "sip:+601112023012@simadmin");
