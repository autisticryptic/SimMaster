...
use crate::connectivity::modems::softstack::vowifi::profile_record::{
    ...
    SmsPolicyRecord, VoicePolicyRecord,
};
...
fn project_config(
    ...
    let record = CarrierProfileRecord {
        ...
        voice: VoicePolicyRecord {
            ...
            preferred_codecs: string_array_at(config, "/services/voice_codecs").unwrap_or_else(
                || {
                    vec![
                        "amr-wb".to_string(),
                        "amr".to_string(),
...
fn project_register(
    ...
    let country_of_origination_format = string_at(register, "/country_of_origination_format")
        .unwrap_or("")
        .to_string();
    let country_needs_pani =
        matches!(country_of_origination_format.as_str(), "PANI" | "BOTH");
